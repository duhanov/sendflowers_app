...
class AdminProductsController < ApplicationController
  ...
  def edit_filter
    @page_title = "Редактировать фильтр"
    @item = F.find_by_id(params[:id])
    if @item.blank?
      flash[:error] = "Не удалось найти фильтр"
      redirect_to "/admin_#{model_controller}/filters"
    else
      if request.post?
        if @item.update_attributes(params[:item])
          #save_product_filters
          flash[:notice] = "Запись отредактирована"
          redirect_to "/admin_#{model_controller}/edit_filter/#{@item.id}"
        end
      end
    end
  end
